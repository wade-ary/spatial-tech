...
Keys:
  ...
  p  toggle continuous palm collection on/off
  t  toggle continuous thumb collection on/off
  m  toggle continuous mouse collection on/off
  q  quit
"""
...
_PALM_IMAGES_DIR = _REPO_ROOT / "data" / "palm"
_THUMB_IMAGES_DIR = _REPO_ROOT / "data" / "thumb"
_MOUSE_IMAGES_DIR = _REPO_ROOT / "data" / "mouse"
_JPEG_QUALITY = 92
_TARGET_FPS = 50.0
...
def save_image(frame, seq: int, label: str) -> Path:
    if label == "click":
        out_dir = _CLICK_IMAGES_DIR
    elif label == "palm":
        out_dir = _PALM_IMAGES_DIR
    elif label == "thumb":
        out_dir = _THUMB_IMAGES_DIR
    elif label == "mouse":
        out_dir = _MOUSE_IMAGES_DIR
    else:
        raise ValueError(f"Unsupported label: {label}")

    out_dir.mkdir(parents=True, exist_ok=True)
    t_ms = int(time.time() * 1000)
    out_path = out_dir / f"{seq:06d}_{t_ms}.jpg"
...
def main() -> None:
    ...
    print("Camera started. Press 'c' (click), 'p' (palm), 't' (thumb), 'm' (mouse), 'q' (quit).")
    sample_seq = 1
    last_saved = "none"
    collecting = False
    current_label = "click"
        ...
        while True:
            ...
            if result.hand_landmarks:
                ...
                if collecting and now >= next_capture_at:
                    ...
                    if crop.size > 0:
                        out_path = save_image(crop, sample_seq, current_label)
            ...
            put_status(
                frame,
                f"Collecting: {status} ({current_label}) | target: 50 fps | saved: {sample_seq - 1}",
            )
            cv2.putText(
                frame,
                f"Last: {last_saved}",
            ...
            cv2.putText(
                frame,
                "Press c(click), p(palm), t(thumb), m(mouse), q(quit)",
                (10, 90),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.6,
                (200, 200, 200),
